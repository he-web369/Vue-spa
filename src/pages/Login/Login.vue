...
import {reqLogin,reqLoginSms,reqSendCode} from '../../api/index'
export default {
  mounted () {
    this.getCaptcha()
  },
  ...
  methods: {
    ...
    async login(){
      ...
      if(this.loginWay){//短信登录
        ...
        const {captcha,name,pwd}=this
        if(!this.name||!this.pwd){
          this.showAlertMsg('用户名或密码必须输入')
          return 
        }else if(!this.captcha){
          this.showAlertMsg('验证码必须输入')
          return 
        }
